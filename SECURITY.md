...
[GitHub Security Advisories](https://github.com/bulletproofsoftware-ai/bulletproof-prd-platform/security/advisories/new)
...
## CLI Proxy Exposure (scripts/cli-proxy.mjs)

The CLI proxy executes the Claude/Gemini CLIs on POSTed prompts and has **no
authentication**. By default it binds `127.0.0.1:3199` (loopback only).

- Anyone who can reach the proxy port can run prompts through your
  authenticated CLI sessions and consume your API/subscription quota.
- Only set `CLI_PROXY_HOST` to a non-loopback interface (e.g. a Docker bridge
  address) on a network where every peer is trusted, and firewall the port
  from everything else.
- Never expose the proxy to a shared LAN or the public internet.
